...
@section('content')
    ...
    {!! Form::model($transaction, ['method' => 'PUT', 'route' => ['admin.transactions.update', $transaction->id], 'files' => true,]) !!}
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($transaction->photo)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$transaction->photo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/'.$transaction->photo) }}"></a>
                    @endif
